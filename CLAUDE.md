...
ユーザーがこのプラグインを初めて使用する場合（`templates/` に `_schema.yaml` 以外のファイルがない場合、またはユーザーが「何ができる？」「使い方は？」と聞いた場合）、以下の案内を日本語で表示する:

```
LawyerSet へようこそ。法律事務所向けの文書処理ツールである。

■ 利用可能なコマンド:
  /typo-check   — 法律文書（DOCX）の誤字脱字・表記揺れを校正する
  /family-tree   — 戸籍謄本PDFから家族関係図（HTML）を生成する
  /template-create — XLSXの裁判所書式をテンプレートとして登録する
  /template-fill   — 登録済みテンプレートにPDFからデータを自動入力する
  /lawsuit-analysis — 訴訟文書を分析しレポートを生成する
  /verify        — プラグインの動作確認

■ まずは /verify で接続確認、または /typo-check で校正をお試しください。
```
