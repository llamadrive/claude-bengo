...
法令名や条番号を指定して、e-Gov 法令 API から条文を即座に取得・表示する。

```
/law-search 民法709条                    ← 条文を即座に表示
/law-search 会社法 第362条               ← 取締役会の権限
「不法行為に関する条文を見せて」           ← キーワードから条文を推定
「さっきの条文を準備書面に引用して」       ← 取得した条文を他コマンドで活用
```

- 主要50法令の条文を即座に参照可能（民法、刑法、会社法、民訴法 等）
- 弁護士がよく使う略称に対応（「民訴法」→ 民事訴訟法、「労基法」→ 労働基準法）
- 頻出トピック（不法行為、消滅時効、相続分 等）から条文範囲を自動推定
- e-Gov 公開 API を使用（認証不要、無料）
