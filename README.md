...
更新は同じく `/plugin install bengo-toolkit@bengo-toolkit` を実行する（marketplace.json の最新バージョンを取得）。
...
バージョンはピン留めされており、更新は Claude Code 標準の `/plugin install bengo-toolkit@bengo-toolkit` 経由で行う（marketplace.json の `version` を見て最新版をキャッシュへ展開）。初回起動時は npx キャッシュがないため各 MCP の初回取得に 30 秒程度かかる（以降はキャッシュから即起動）。
...
for d in ~/cases/*/; do
  ...
  if [ -f "$d/.claude-bengo/audit.jsonl" ]; then
    (cd "$d" && python3 ~/.claude/plugins/cache/bengo-toolkit/bengo-toolkit/{VERSION}/skills/_lib/audit.py export --format csv) \
...
```bash
python3 ~/.claude/plugins/bengo-toolkit/skills/_lib/audit.py export --format csv --since 2026-04-01 > audit-2026-04.csv
```
...
Claude Code 内で `/plugin` → **Marketplaces タブ** → bengo-toolkit を選択 →
...
```
/plugin marketplace update bengo-toolkit      ← catalogue を refresh
/plugin uninstall bengo-toolkit@bengo-toolkit  ← 削除
/plugin install bengo-toolkit@bengo-toolkit    ← 最新版で再取得
