...
| `~/.claude-bengo/`（admin lock・HMAC 鍵・consent） | そのまま永続化 | セッション毎にリセット、**`~/.claude-bengo/` を明示的にフォルダ選択する必要がある** |
| 会話の途中で別案件フォルダへ切替 | `cd ~/cases/別案件` で即可能 | 不可。新しいセッションを開始して別フォルダを選択し直す |
