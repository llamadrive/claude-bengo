...
| Claude Desktop アプリ（チャットモード、Cowork モード外） | ⚠ 部分対応 | MCP サーバーのみ手動設定で動作可能だが、本プラグインの主要機能（スラッシュコマンド・監査ログ・同意ゲート等）は動作しない |
...
| `~/.claude-bengo/`（admin lock・HMAC 鍵・consent） | そのまま永続化 | Cowork の VM は毎セッション初期化される。ホスト側 `~/.claude-bengo/` を**明示的に "Select folders" で選ばない限り**、admin lock・HMAC 鍵・同意記録は毎回やり直しになる |
...
1. **案件フォルダと `~/.claude-bengo/` の両方を Cowork の "Select folders" で事前に選ぶ**
   ...
   - `~/.claude-bengo/` を選ばないと admin lock・監査 HMAC 鍵・同意記録がセッション毎にリセットされる（`/consent admin-setup` を毎回やり直すことになる）
...
4. **初回同意は lawyer ではなく管理者が実施**
   - `/consent admin-setup --passphrase "..."`（管理者、一度だけ、事務所共有パスフレーズ）
   - `/consent grant --answer "同意する" --admin-passphrase "..."`（管理者、セッション開始時に行う）
   - これは Claude Code / Cowork 共通の設計だが、Cowork では `~/.claude-bengo/` を mount しないと毎回やり直しになる
...
|------|-------------|----------------|-----------|
| `/family-tree`, `/typo-check`, `/lawsuit-analysis`, `/template-fill`, `/template-create` | 任意パスの PDF / DOCX / XLSX を即読取 | 選択済みフォルダ内に限定 | VM mount 範囲 |
...
| 同意・admin lock（`~/.claude-bengo/global.json`） | 永続化 | **`~/.claude-bengo/` mount 必須**、mount なしでは毎回再設定 | VM の `~/` は ephemeral |
...
Claude Code / Cowork が GitHub からリポジトリを取得し、自動的にプラグインとして登録する。インストール完了後、Claude Code / Cowork を再起動すれば 3 つの MCP サーバが起動する（初回 30 秒程度）。

更新は同じく `/plugin install claude-bengo@claude-bengo` を実行する（marketplace.json の最新バージョンを取得）。
