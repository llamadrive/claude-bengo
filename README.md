...
Claude Code が GitHub からリポジトリを取得し、自動的にプラグインとして登録する。インストール完了後、Claude Code を再起動すれば 4 つの MCP サーバが起動する（初回 30 秒程度）。

更新は `/bengo-update` を実行する（署名付きタグ経由で安全に更新）。
...
事案データは端末ごとに分離される。同一ユーザーが複数端末で同じ事案に作業する場合は、`~/.claude-bengo/` を暗号化ボリューム（FileVault / BitLocker）経由で同期するか、`CLAUDE_BENGO_ROOT` を NFS / SMB マウントの共有ボリュームに向ける。
...
法律事務所への導入時は `RUNBOOK.md` に従って 15〜20 分の動作確認を実施する。MCP サーバの疎通、事案作成、機密スキルの matter ガード、監査ログのハッシュチェーン検証と改ざん検知までを一通り確認できる。
...
| `/template-install` | 同梱書式（23種・9カテゴリ：家事/相続/交通事故/民訴/労働/破産/刑事/一般民事/汎用）をインストール | ✅ |
...
│   ├── _lib/                     # 共通ライブラリ（audit, matter, templates）
