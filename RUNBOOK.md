...
- [ ] `/plugin install bengo-toolkit@bengo-toolkit` で更新（再起動後 /verify）
...
/plugin uninstall bengo-toolkit@bengo-toolkit  ← 一度削除
/plugin install bengo-toolkit@bengo-toolkit    ← 新バージョンで再取得
/reload-plugins
```
...
d['plugins'].pop('bengo-toolkit@bengo-toolkit', None)
r.write_text(json.dumps(d, indent=2) + chr(10))
print('removed')
"
```

Claude Code を起動しなおして `/plugin install bengo-toolkit@bengo-toolkit` を再実行する。
