...
**失敗時の診断:**
- `MCP サーバが見つからない` → `npm install -g @knorq/xlsx-mcp-server@2.0.0 @knorq/docx-mcp-server@2.0.0 @knorq/html-report-server@2.0.0` を先に実行する
...
log = m / 'audit.jsonl'
text = log.read_text()
tampered = text.replace('校正', 'TAMPERED')
...
**想定結果:** `git fetch --tags` → 最新タグを探す → **署名付きタグがない場合は中止**。v2.0.1 タグが `git tag -s` で署名されて GitHub に push されていれば、signer を表示して確認を求める。
