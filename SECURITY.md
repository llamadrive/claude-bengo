...
本プラグイン (`bengo-toolkit`、旧 `claude-bengo`) は日本の法律事務所でクライアント機密文書を扱うことを想定しているため、一般的な OSS よりも厳しい前提でセキュリティを扱う。
...
- **ユーザースコープテンプレートへの PII 混入防止**: `skills/_lib/pii_scan.py` が `/template-promote` 実行時にハードブロックする。case スコープ → user スコープ昇格前に PII 検出 → 拒否。
