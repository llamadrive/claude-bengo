...
日本の法律事務所の安全管理措置（個人情報保護法第23条）が要求する処理記録の
保管を補助し、また弁護士法第23条（秘密保持義務）の履行を裏付ける証跡として、
本プラグインで処理した文書のメタデータを追記専用ログとして記録する。

本ログ単体で個人情報保護法・弁護士法上のいずれの義務も「履行された」とは
ならない点に留意する。委託先監督（個人情報保護法第25条）は別途、Anthropic
を含む処理委託先に対する選定・契約・モニタリングの 3 要素で履行する必要が
あり、本プラグインの README に §25 チェックリスト雛形を別途用意している。
