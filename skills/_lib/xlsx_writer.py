...
class Workbook:
    ...
    def write_cell(
        ...
        バリデーション:
            - float の inf / NaN は Excel が破損ファイルと判定するため拒否
            - 文字列内の NUL バイト (\\x00) は XML 1.0 で許可されないため拒否
        """
        ...
        if isinstance(value, float):
            if not _math.isfinite(value):
                raise ValueError(
                    f"write_cell: 非有限の float は XLSX に書けない（受信: {value!r} at row={row}, col={col}）"
                )
        elif isinstance(value, str):
            if "\x00" in value:
                raise ValueError(
                    f"write_cell: 文字列に NUL バイトが含まれる（XML 1.0 で禁止）at row={row}, col={col}"
                )
...
def _self_test() -> int:
    ...
    guard_fail = 0
    try:
        wb2.write_cell(1, 1, float("inf"))
        print("  [FAIL] inf の拒否")
        guard_fail += 1
    except ValueError:
        pass
    try:
        wb2.write_cell(1, 1, float("nan"))
        print("  [FAIL] NaN の拒否")
        guard_fail += 1
    except ValueError:
        pass
    try:
        wb2.write_cell(1, 1, "hello\x00world")
        print("  [FAIL] NUL バイト文字列の拒否")
        guard_fail += 1
    except ValueError:
        pass
    if guard_fail == 0:
        print("  [PASS] V26-001 ガード: inf/NaN/NUL を拒否")

    print("xlsx_writer: 4/4 passed")
