...
閲覧方法:
  ...
  📱 Claude Desktop / Cursor など MCP Apps 対応クライアント
     上記ファイル名をチャットに貼り、agent-format MCP の
     render_agent_file ツールで直接インライン表示できる。

  💻 Claude Code CLI やその他の環境
     1. https://knorq-ai.github.io/agent-format/ を開く
     2. `family_tree_{YYYY-MM-DD}.agent` をドラッグ & ドロップ
     3. 右上「Open in browser」ボタンで共有リンク発行
     4. 「PDF」ボタンで裁判所提出用 A3 横の HTML を保存、
        ブラウザで開いて ⌘P で PDF 化

  🔒 完全オフライン印刷
     web viewer の「PDF」ボタン出力 HTML は自己完結、オフラインで開ける。
```

**なぜ HTML を直接出力しないか:** agent-format web viewer の「PDF」ボタンが同じ HTML を生成する。Claude Code が毎回出力すると token を浪費し、ファイル数も増える。`.agent` が canonical で、印刷 HTML は web viewer 側で on-demand 生成するのが単一責務設計。
