...
2. **尊属（祖父母世代）の扱い**
   - 戸籍の【父】【母】欄や従前戸籍欄から祖父母等の直系尊属が抽出できた場合、以下を確認する:
     ...
     (b) 拡張書式（事案整理・相続人確定作業用）: 祖父母・兄弟姉妹等すべてを含める
         出力: variant なし（または "extended"）
     ```
   - 子・孫がおらず、尊属が相続人となる可能性がある場合は (a) でも尊属を描画する（民法 889 条）。
   - v3.1.0 〜: @agent-format renderer は `data.persons` に含まれる人物を**全員**描画する設計に変更された。「書式の違い = データの違い」で、レンダラ側のフィルタリングではない。上記 (a) は Step 3 で heirs-only な persons 配列を作り、(b) は全員を含めた配列を作ることで表現する。
...
| (b) 拡張（事案整理） | 全員（祖父母・兄弟姉妹・甥姪・配偶者の親族等、戸籍から抽出された person 全員） | 省略または `"extended"` |

`variant` は視覚スタイル（Japanese 法定体裁・PDF ボタン等）のみを制御し、レンダラが人物をフィルタしない — 上表通り persons 配列の中身を使い分けること。
